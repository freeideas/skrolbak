...
```bash
{{UV_BINARY}} run --script ./the-system/scripts/test.py {{TEST_FILE_PATH}}
```
...
**Compiler location:** Always use the portable compiler in `./tools/compiler/`. Do **not** rely on PATH. See `./the-system/prompts/DOWNLOAD_COMPILER.md` for supported compilers and their paths.
...
**IMPORTANT:** If you encounter errors with tools in `./the-system/`, do NOT try to fix them. Report the error and stop. The system handles platform-specific binaries automatically.
