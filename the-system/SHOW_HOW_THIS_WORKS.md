...
Create a detailed execution hierarchy that documents the complete flow of the **@the-system/ build system itself** (not any project using it). Reference `@the-system/00_OVERVIEW.md` to understand the overall context and workflow.

## Instructions

1. **Start with the root script**: Read `@the-system/scripts/software-construction.py` and create a hierarchy starting from the `main()` function.
...
3. **Recursively break down called Python scripts**:
   ...
   - All scripts are within `@the-system/scripts/` directory
...
4. **Document all AI prompt executions**:
   - Whenever a prompt is executed (via `run_ai_prompt()` with a `.md` file path), create a subtree that:
     - Reads the actual `.md` prompt file at that path within `@the-system/prompts/`
...
5. **Format the hierarchy**:
   ...
   - Focus scope: only on @the-system/ internals
