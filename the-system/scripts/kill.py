...
"""
Kill any processes running from ./the-system/ directories.

Targets the main repo plus any sandboxes (e.g., ./tmp/*/the-system).
...
def discover_the_system_dirs() -> list[Path]:
    """Find all the-system directories (main repo + sandboxes in ./tmp/)."""
    targets: list[Path] = []

    main_dir = PROJECT_ROOT / 'the-system'
    if main_dir.exists():
        targets.append(main_dir.resolve())

    tmp_dir = PROJECT_ROOT / 'tmp'
    if tmp_dir.exists():
        for candidate in tmp_dir.glob('**/the-system'):
            if candidate.is_dir():
                try:
...
def main() -> int:
    ...
    print("KILLER Searching for processes under ./the-system/ ...")
    targets = discover_the_system_dirs()
    if not targets:
        print("  No the-system directories found; nothing to do.")
        return 0
