...
"""
Push the-system to its GitHub repository.

This script handles the .disabled.git* convention: the-system keeps its git
...
Usage:
    the-system_push.py [commit message]
...
# Change to the-system directory (one level up from scripts/)
...
def find_disabled_git_files():
    """Find all .disabled.git* files/directories in the-system root."""
    disabled = []
...
def find_git_files():
    """Find all .git* files/directories in the-system root."""
    git_files = []
    for item in THE_SYSTEM_DIR.iterdir():
